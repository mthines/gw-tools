...
import { multiSelect, type SelectSection, type TerminalIO } from './interactive-select.ts';
...
function createMockIO(keys: Uint8Array[]): { io: TerminalIO; output: string[] } {
  let keyIndex = 0;
...
Deno.test('multiSelect - protected items can be selected', async () => {
  ...
  // space -> select it
  // enter -> confirm
  const keys = encodeKeys('down', 'down', 'down', 'down', 'down', 'space', 'enter');
  const { io } = createMockIO(keys);
...
Deno.test('multiSelect - multiple items across sections', async () => {
  ...
  // enter -> confirm
  const keys = encodeKeys('down', 'space', 'down', 'down', 'down', 'space', 'enter');
  const { io } = createMockIO(keys);
