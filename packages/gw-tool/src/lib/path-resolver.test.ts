...
Deno.test('validatePathExists - throws for non-existent path', async () => {
  ...
  try {
    ...
    await assertRejects(() => validatePathExists(nonExistentPath, 'file'), Error, 'File not found');
  } finally {
    await repo.cleanup();
  }
});
...
Deno.test('validatePathExists - throws when path is wrong type (file vs directory)', async () => {
  ...
  try {
    ...
    const filePath = join(repo.path, 'test.txt');
    await assertRejects(() => validatePathExists(filePath, 'directory'), Error, 'Path exists but is not a directory');
  } finally {
    await repo.cleanup();
  }
});
...
Deno.test('validatePathExists - throws when directory expected but file exists', async () => {
  ...
  try {
    ...
    await assertRejects(() => validatePathExists(repo.path, 'file'), Error, 'Path exists but is not a file');
  } finally {
    await repo.cleanup();
  }
});
