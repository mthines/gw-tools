 ...
 */
export async function getWorktreeAgeDays(
  worktreePath: string,
): Promise<number> {
  const ages: number[] = [];
 ...
 */
export async function hasBranchUnpushedCommits(
  branchName: string,
): Promise<boolean> {
  // First check if there's a remote tracking branch
  const trackingCmd = new Deno.Command("git", {
 ...
 */
export async function findOrphanBranches(
 ...
 */
export async function pruneOrphanBranches(
  defaultBranch: string,
): Promise<number> {
...
export async function deleteLocalBranch(
  ...
  if (errorOutput.includes("checked out")) {
    return {
      success: false,
      message:
        `Cannot delete branch '${branchName}' - it is checked out in another worktree.`,
    };
  }
...
export async function getStagedFileContent(
  ...
  if (code !== 0) {
    const errorMsg = new TextDecoder().decode(stderr).trim();
    throw new Error(
      `Failed to get staged file content for '${filePath}': ${errorMsg}`,
    );
  }
...
export async function copyStagedFiles(
  ...
  if (files && files.length > 0) {
    ...
    if (filesToCopy.length === 0) {
      throw new Error(
        `None of the specified files are staged: ${files.join(", ")}`,
      );
    }
  }
