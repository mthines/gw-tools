  ...
  try {
    ...
    try {
      ...
      assertEquals(
        date.length > 0,
        true,
        "Should return a non-empty date string",
      );
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        date,
        "",
        "Should return empty string for nonexistent branch",
      );
    } finally {
      cwd.restore();
    }
...
Deno.test("copyStagedFiles - throws when no staged files", async () => {
  ...
  try {
    ...
    await assertRejects(
      () => copyStagedFiles(sourceRepo.path, targetRepo.path),
      Error,
      "No staged files to copy",
    );
  } finally {
    await sourceRepo.cleanup();
    await targetRepo.cleanup();
