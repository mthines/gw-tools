 ...
 */
export async function assertDirExists(path: string, message?: string): Promise<void> {
  try {
    const stat = await Deno.stat(path);
    if (!stat.isDirectory) {
 ...
 */
export async function assertPathNotExists(path: string, message?: string): Promise<void> {
  try {
 ...
 */
export async function assertFileContent(path: string, expected: string, message?: string): Promise<void> {
 ...
 */
export async function assertWorktreeExists(repoPath: string, worktreeName: string): Promise<void> {
  const output = await captureCommand('git', ['worktree', 'list', '--porcelain'], repoPath);
 ...
 */
export async function assertWorktreeNotExists(repoPath: string, worktreeName: string): Promise<void> {
  const output = await captureCommand('git', ['worktree', 'list', '--porcelain'], repoPath);
  const worktrees = parseWorktreeList(output);
  const expected = join(repoPath, worktreeName);
 ...
 */
export async function assertBranchExists(repoPath: string, branchName: string): Promise<void> {
 ...
 */
export async function assertBranchNotExists(repoPath: string, branchName: string): Promise<void> {
  const { code } = await new Deno.Command('git', {
    args: ['rev-parse', '--verify', branchName],
    cwd: repoPath,
    stdout: 'null',
 ...
 */
async function captureCommand(cmd: string, args: string[], cwd?: string): Promise<string> {
