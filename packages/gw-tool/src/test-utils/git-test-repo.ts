...
export class GitTestRepo {
  ...
  async createCommit(message: string): Promise<void> {
    ...
    await this.runCommand('git', ['commit', '-m', message, '--allow-empty'], this.path);
  }
  ...
  async listWorktrees(): Promise<string[]> {
    const output = await this.captureCommand('git', ['worktree', 'list', '--porcelain'], this.path);
   ...
   */
  async runCommand(cmd: string, args: string[], cwd?: string): Promise<void> {
   ...
   */
  private async captureCommand(cmd: string, args: string[], cwd?: string): Promise<string> {
    const process = new Deno.Command(cmd, {
      args,
      cwd: cwd || this.path,
      stdout: 'piped',
