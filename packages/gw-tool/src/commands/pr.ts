...
function parsePrIdentifier(
  identifier: string,
): { prNumber: number; owner?: string; repo?: string } | null {
 ...
 */
async function getCurrentRepo(): Promise<
...
async function fetchPrInfo(prNumber: number): Promise<PrInfo | null> {
  try {
    ...
    if (code !== 0) {
      ...
      if (
        errorMsg.includes("Could not resolve") || errorMsg.includes("not found")
      ) {
        return null;
      }
...
export async function executePr(args: string[]): Promise<void> {
  ...
  if (!(await isGhInstalled())) {
    ...
    console.log(
      "The gw pr command requires the GitHub CLI to fetch PR information.",
    );
    console.log("");
  ...
  if (prIdResult.owner && prIdResult.repo) {
    ...
    if (
      ...
      output.error(
        `PR URL is for repository '${prIdResult.owner}/${prIdResult.repo}'`,
      );
      console.log(
        `But you're currently in '${currentRepo.owner}/${currentRepo.repo}'`,
      );
      console.log("");
      console.log(
        "Hint: Use just the PR number if you want to fetch from the current repo:",
      );
  ...
  if (!prInfo) {
    ...
    console.log("");
    console.log(
      "If this is a private repository, ensure you are authenticated:",
    );
    console.log("  gh auth login\n");
    Deno.exit(1);
  }
    ...
    if (stat.isDirectory || stat.isFile) {
      ...
      if (isValidWorktree) {
        ...
        output.warning(
          ...
          } already exists but is not a valid worktree.`,
        );
        console.log(
  ...
  console.log(
    output.dim(`  git fetch origin pull/${prNumber}/head:${branchName}`),
  );
  console.log("");
  ...
  output.success(
    `Worktree ${output.bold(`"${worktreeName}"`)} created for PR #${prNumber}`,
  );
