...
export async function executeUpdate(args: string[]): Promise<void> {
  ...
  try {
    ...
    if (parsed.merge && parsed.rebase) {
      output.error(
        "Cannot use both --merge and --rebase flags. Please choose one.",
      );
      Deno.exit(1);
    }
    ...
    if (await isDetachedHead(currentPath)) {
      output.error(
        "Cannot update: currently in detached HEAD state. Checkout a branch first.",
      );
      Deno.exit(1);
    }
    ...
    console.log(
      `Fetching latest ${output.bold(targetBranch)} from ${
        output.bold(parsed.remote)
      }...`,
    );

    const { startPoint, fetchSucceeded, message } = await fetchAndGetStartPoint(
      targetBranch,
      parsed.remote,
    );
    ...
    if (fetchSucceeded) {
      ...
      if (parsed.branch && !noRemoteConfigured) {
        ...
        console.log(
          `  • Branch ${output.bold(targetBranch)} doesn't exist on remote`,
        );
        ...
        console.log(
          `  2. Verify the branch exists: ${
            output.bold(`git ls-remote ${parsed.remote} ${targetBranch}`)
          }`,
        );
        console.log(
          `  3. Use a different source branch: ${
            output.bold(`gw update --from <branch>`)
          }`,
        );
        console.log(
          `  4. Update from default branch: ${output.bold(`gw update`)}`,
        );
        console.log("");
        Deno.exit(1);
      }
    ...
    const operationMessage = strategy === "merge"
      ? `${strategyVerb} ${output.bold(startPoint)} ${strategyPrep} ${
        output.bold(currentBranch)
      }`
      : `${strategyVerb} ${output.bold(currentBranch)} ${strategyPrep} ${
        output.bold(startPoint)
      }`;
    ...
    const result = strategy === "merge"
      ? await mergeBranch(currentPath, startPoint)
      : await rebaseBranch(currentPath, startPoint);
    ...
    if (result.success) {
      if (result.message === "Already up to date") {
        ...
        console.log(
          output.dim(`${output.checkmark()} ${strategyPastTense} successfully`),
        );
        console.log("");
        output.success(
          `Updated ${output.bold(currentBranch)} with latest changes from ${
            output.bold(targetBranch)
          }`,
        );
      ...
      console.log("");
      output.error(
        `${strategy === "merge" ? "Merge" : "Rebase"} conflict detected`,
      );
      console.log("");
      ...
      console.log("");
      output.error(
        result.message || `${strategy === "merge" ? "Merge" : "Rebase"} failed`,
      );
      Deno.exit(1);
    }
  } catch (error) {
