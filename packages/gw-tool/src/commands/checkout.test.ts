...
Deno.test("checkout command - --progress=json emits create-worktree start and end events on stderr", async () => {
  ...
  try {
    ...
    const startEvent = events.find((e) =>
      e.stage === "create-worktree" && e.status === "start"
    );
    const endEvent = events.find((e) =>
      e.stage === "create-worktree" && e.status === "end"
    );

    assertEquals(
      startEvent !== undefined,
      true,
      "should emit create-worktree start event",
    );
    assertEquals(
      endEvent !== undefined,
      true,
      "should emit create-worktree end event",
    );
...
Deno.test("checkout command - --progress=json end event has durationMs", async () => {
  ...
  try {
    ...
    assertEquals(
      typeof endEvent?.durationMs,
      "number",
      "end event must have numeric durationMs",
    );
  } finally {
    await repo.cleanup();
  }
...
Deno.test("checkout command - without --progress=json no JSON appears on stderr", async () => {
  ...
  try {
    ...
    const jsonLines = capture.getOutput().trim().split("\n").filter((l) =>
      l.startsWith("{")
    );
    assertEquals(
      jsonLines.length,
      0,
      "no JSON events should appear on stderr without --progress=json",
    );
  } finally {
    await repo.cleanup();
  }
...
Deno.test("checkout command - --progress=json hook events include hook, of, command fields", async () => {
  ...
  try {
    ...
    const config = createConfigWithHooks(repo.path, undefined, [
      'echo "hook ran"',
    ]);
    await writeTestConfig(repo.path, config);
    ...
    const events = lines.map((l) => JSON.parse(l));
    const hookStartEvent = events.find((e) =>
      e.stage === "post-checkout-hooks" && e.status === "start"
    );

    assertEquals(
      hookStartEvent !== undefined,
      true,
      "hook start event must exist",
    );
    ...
    assertEquals(
      typeof hookStartEvent?.command,
      "string",
      "command field must be a string",
    );
    assertMatch(
      hookStartEvent?.command as string,
      /echo/,
