...
Deno.test('checkout command - switches branch when worktree at path has different branch', async () => {
  ...
  try {
    ...
    await repo.runCommand('git', ['worktree', 'add', worktreePath, 'target-branch'], repo.path);
    ...
    await repo.runCommand('git', ['-C', worktreePath, 'checkout', 'squatter-branch'], repo.path);
    ...
    try {
      ...
      const branchResult = await branchCmd.output();
      const currentBranch = new TextDecoder().decode(branchResult.stdout).trim();
      assertEquals(currentBranch, 'target-branch', 'worktree should have switched to target-branch');

      // Verify navigation was signalled to the worktree path
      const home = Deno.env.get('HOME') || Deno.env.get('USERPROFILE') || '';
      const navFile = join(home, '.gw', 'tmp', 'last-nav');
      const navPath = await Deno.readTextFile(navFile);
      assertEquals(navPath, worktreePath, 'should navigate to the worktree path');
...
Deno.test('checkout command - errors when worktree at path has uncommitted changes', async () => {
  ...
  try {
    ...
    await repo.runCommand('git', ['worktree', 'add', worktreePath, 'target-branch-dirty'], repo.path);

    // Switch the worktree to squatter-branch-dirty
    await repo.runCommand('git', ['-C', worktreePath, 'checkout', 'squatter-branch-dirty'], repo.path);
    ...
    try {
      ...
      assertEquals(exitCode, 1, 'should fail with exit code 1 when uncommitted changes exist');
