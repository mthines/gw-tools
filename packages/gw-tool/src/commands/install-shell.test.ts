  ...
  try {
    ...
    try {
      ...
      assertStringIncludes(
        ...
        "Should add integration comment to .zshrc"
      );
      assertStringIncludes(
        zshrcContent,
        "source ~/.gw/shell/integration.zsh",
        "Should add source line to .zshrc"
      );
      ...
      assertStringIncludes(
        scriptContent,
        "gw() {",
        "Should create gw function in integration script"
      );
      assertStringIncludes(
        scriptContent,
        'if [[ "$1" == "cd" ]];',
        "Should handle cd command in integration script"
      );
    } finally {
      // Restore SHELL
      if (originalShell) {
  ...
  try {
    ...
    try {
      ...
      assertStringIncludes(
        bashrcContent,
        "# gw-tools shell integration",
        "Should add integration comment to .bashrc"
      );
      assertStringIncludes(
        bashrcContent,
        "source ~/.gw/shell/integration.bash",
        "Should add source line to .bashrc"
      );
  ...
  try {
    ...
    try {
      ...
      assertStringIncludes(
        functionContent,
        "function gw",
        "Should create gw function"
      );
      assertStringIncludes(
        functionContent,
        'if test "$argv[1]" = "cd"',
        "Should handle cd command in fish function"
      );
    } finally {
      // Restore SHELL
      if (originalShell) {
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        finalLineCount,
        initialLineCount,
        "Should not duplicate integration lines when already installed"
      );
    } finally {
      if (originalShell) {
        Deno.env.set("SHELL", originalShell);
  ...
  try {
    ...
    try {
      ...
      assertStringIncludes(
        ...
        "Should use custom command in function"
      );
    } finally {
      if (originalShell) {
        Deno.env.set("SHELL", originalShell);
