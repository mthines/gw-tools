...
Deno.test("install-shell - outputs zsh shell function to stdout", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      ...
      "Should include integration comment",
    );
  } finally {
    if (originalShell) {
...
Deno.test("install-shell - outputs fish shell function to stdout", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      stdout || "",
      "function gw",
      "Should output fish gw function",
    );
    assertStringIncludes(
      stdout || "",
      'if test "$argv[1]" = "cd"',
      "Should handle cd command in fish",
    );
  } finally {
    if (originalShell) {
      Deno.env.set("SHELL", originalShell);
...
Deno.test("install-shell - exits with error for unsupported shell", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      ...
      "Should show which shell is unsupported",
    );
    assertStringIncludes(
      combinedOutput,
...
Deno.test("install-shell - does not require HOME for default mode", async () => {
  ...
  try {
    ...
    const { stdout, exitCode } = await withMockedExit(
      () => executeInstallShell([]),
      { captureOutput: true },
    );
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        scriptExists,
        false,
        "Should delete legacy integration script",
      );

      // Verify source line was removed from .zshrc
      const content = await Deno.readTextFile(zshrcPath);
      assertEquals(
        content.includes("gw-tools shell integration"),
        false,
        "Should remove integration comment",
      );
      assertEquals(
        content.includes("source ~/.gw/shell/integration"),
        false,
        "Should remove source line",
      );
...
Deno.test("install-shell - removes eval-based integration with --remove", async () => {
  ...
  try {
    ...
    try {
      ...
      const content = await Deno.readTextFile(zshrcPath);
      assertEquals(
        content.includes("gw install-shell"),
        false,
        "Should remove eval line",
      );
      assertStringIncludes(content, "export PATH", "Should keep other content");
      assertStringIncludes(
        content,
...
Deno.test("install-shell - outputs custom command name function", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      stdout || "",
      "gw-dev() {",
      "Should create function with custom name",
    );
    assertStringIncludes(
      stdout || "",
      "deno run --allow-all main.ts",
  ...
  try {
    ...
    try {
      ...
      assertEquals(
        funcExists,
        false,
        "Should delete legacy fish function file",
      );
...
Deno.test("zsh function - includes directory existence check before cd", () => {
  ...
  assertStringIncludes(
    output,
    'if [[ -d "$nav_path" ]];',
    "Should check path exists before cd",
  );
});
